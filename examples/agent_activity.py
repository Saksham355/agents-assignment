...
from .backchanneling import BackchannelingFilter
...
class AgentActivity(RecognitionHooks):
    def __init__(self, agent: Agent, sess: AgentSession) -> None:
        ...
        self._backchanneling_filter: BackchannelingFilter | None = None
        if sess.options.filter_backchanneling:
            self._backchanneling_filter = BackchannelingFilter(
                backchanneling_words=sess.options.backchanneling_words,
                interrupt_words=sess.options.interrupt_words,
            )
    ...
    def _interrupt_by_audio_activity(self, *, transcript: str | None = None) -> None:
        ...
        # Check backchanneling filter when we have a transcript
        if (
            self._backchanneling_filter is not None
            and transcript
            and self._current_speech is not None
            and not self._current_speech.interrupted
        ):
            if self._backchanneling_filter.should_filter_interruption(transcript):
                logger.debug(
                    "backchanneling detected, skipping interruption",
                    extra={"transcript": transcript}
                )
                # If we were paused (pending interruption), resume immediately
                if (
                    self._paused_speech is not None
                    and self._session.output.audio
                    and self._session.output.audio.can_pause
                ):
                    logger.debug("resuming after backchanneling detection")
                    self._session.output.audio.resume()
                    self._session._update_agent_state("speaking")
                    # Cancel any pending false interruption timer
    ...
    def on_interim_transcript(self, ev: stt.SpeechEvent, *, speaking: bool | None) -> None:
        ...
        if transcript_text and self._turn_detection not in (
            ...
            # Pass transcript for backchanneling detection
